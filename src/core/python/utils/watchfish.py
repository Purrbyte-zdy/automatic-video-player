...
class Timer:
    ...
    def __init__(self, force_mode=False) -> None:
        ...
        self.starting_time = self.weekday_cfg.get("startTime")

        stopping_method = self.weekday_cfg.get("closingMethod")
        stopping_time_lst = self.weekday_cfg.get("endTime")
        stopping_time_str: str

        # BUG: STRANGERS FORCE MODE CAN CAUSE UNEXPECTED STOP TIME, MAY CAUSE BY TOO MANY NEW OBJECTS. SPILT MIGHT FIX THIS PROBLEM.
    ...
    def time(delta=0, minute=0) -> Tuple[int, int, int, str, int, int, int]:
        ...
        All types were int, WeekdayName such as 'Monday'.
        """
    ...
    def is_time_to_start(self) -> bool:
        now_time = (Timer.time()[4], Timer.time()[5])
        if (self.starting_time[0][0] <= now_time[0] <= self.starting_time[1][0] and
            self.starting_time[0][1] <= now_time[1] <= self.starting_time[1][1]):
            return True
        return False
    ...
    def is_time_for_stop(self) -> bool:
        """
        Check time for stop.
        Return True when is time for stop.
        """
        now_dt = datetime.now()
